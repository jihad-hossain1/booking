      ...
      <div>
        ...
        <div className="hidden md:grid md:grid-cols-7 gap-0 ">
          {calendarUtils.constantWeek().map((day, index) => (
            <div
              key={day}
              className="bg-gray-50 p-4 max-sm:p-2 text-center font-medium text-gray-700 border-b border-r border-gray-200"
            >
              {day}
            </div>
          ))}
        ...
        <div className="grid  md:grid-cols-7 gap-0 ">
          {weekDays.map((day, index) => {
            ...
            return (
              <div
                key={index}
                className={`min-h-[200px] p-2 border-r border-b border-gray-200 ${
              ...
              >
                ...
                <div className="space-y-1">
                  {dayBookings.map((booking) => (
                    <div
                      key={booking.id}
                      className={`p-2 rounded border-l-2 text-xs ${calendarUtils.getBookingColor(
                        booking.resource,
                        RESOURCES
                      )}`}
                    >
                      ...
                      <div className="flex items-center space-x-1 mt-1">
                        ...
                        <span>
                          {calendarUtils.formatTime(booking.startTime)} -{" "}
                          {calendarUtils.formatTime(booking.endTime)}
                        </span>
                      </div>
                      <div className="flex items-center space-x-1 mt-1">
